...
const ImageModal = ({
  ...
  return createPortal(
    <div
      className="fixed inset-0 bg-black/80 backdrop-blur-sm flex items-center justify-center z-[9999] p-2"
      onClick={onClose}
    >
      <div
      ...
      >
        <Button
          variant="ghost"
          size="icon"
          className="absolute top-2 right-2 text-white z-20 h-10 w-10 hover:bg-white/10"
          onClick={onClose}
        >
          <X className="h-6 w-6" />
        </Button>
        <Button
          variant="ghost"
          size="icon"
          className="absolute left-0 sm:left-4 top-1/2 -translate-y-1/2 text-white z-20 h-16 w-16 hover:bg-white/10 disabled:opacity-20"
        ...
        <div className="relative flex flex-col items-center justify-center gap-4">
          ...
          {!isSelectionComplete && (
            <div className="absolute top-4 right-4 flex items-center gap-2 bg-white/60 rounded-full p-2 shadow-md">
              <Heart
                className={`h-6 w-6 ${isSelected ? "fill-red-500 text-red-500" : "text-gray-800"}`}
              />
              <button
        ...
        <Button
          variant="ghost"
          size="icon"
          className="absolute right-0 sm:right-4 top-1/2 -translate-y-1/2 text-white z-20 h-16 w-16 hover:bg-white/10 disabled:opacity-20"
        ...
        <CardHeader>
          ...
          <CardDescription>
            {isSelectionComplete
              ...
              : "Clique numa imagem para a ver em detalhe e selecionar as suas favoritas."}
          </CardDescription>
        </CardHeader>
      </Card>
      <div className="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-4 lg:grid-cols-5 gap-2 md:gap-4">
        {gallery.images.map((imageUrl, idx) => (
          <div
            key={imageUrl}
            className="aspect-square overflow-hidden rounded-lg cursor-pointer group relative"
            onClick={() => setModalImageIndex(idx)}
            onMouseEnter={() =>
              preloadImage(optimizeCloudinaryUrl(imageUrl, "f_auto,q_auto,w_1600"))
            }
          >
            <img
  ...
  const fetchGalleries = async () => {
    ...
    try {
      ...
      toast({
        variant: "destructive",
        title: "Erro",
        description: "Não foi possível carregar as suas galerias.",
      });
    } finally {
      setIsLoading(false);
    }
