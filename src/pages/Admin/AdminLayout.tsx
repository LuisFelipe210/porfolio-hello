...
const AdminLayout = () => {
    ...
    return (
        <div className="flex h-screen overflow-hidden bg-black text-white">
            ...
            {!isMobile && (<aside className="h-full bg-black/50 backdrop-blur-lg border-r border-white/10 flex flex-col relative z-20 transition-all duration-300 ease-in-out" style={{ width: isSidebarOpen ? sidebarWidth : '0', minWidth: isSidebarOpen ? sidebarWidth : '0', opacity: isSidebarOpen ? 1 : 0, transform: isSidebarOpen ? 'translateX(0)' : `translateX(-${sidebarWidth})`, padding: isSidebarOpen ? '1rem' : '0', }}><div className="flex items-center gap-3 p-4 border-b border-white/10 mb-4"><img src={Logo} alt="Hellô Borges" className="h-10 w-auto" /><span className="text-xl font-semibold text-white">Painel</span></div><NavLinks className="flex-1" /><div className="mt-auto p-4 border-t border-white/10"><Button variant="outline" className="w-full text-red-500 hover:bg-red-500/10 hover:text-red-400 border-red-500/50 transition-colors duration-200 rounded-2xl p-6 text-base" onClick={handleLogout}><LogOut className="mr-3 h-5 w-5" /> Sair</Button></div></aside>)}
            <div className="flex-1 flex flex-col overflow-hidden">
                ...
                <main className="relative z-10 flex-1 p-4 sm:p-6 md:p-8 overflow-y-auto"><Outlet /></main>
            </div>
        </div>
    );
};
