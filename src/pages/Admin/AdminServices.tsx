                    ...
                    <Card className="bg-black/70 backdrop-blur-md rounded-3xl shadow-md border-white/10">
                        <CardContent className="p-6">
                            <form onSubmit={handleSave} className="space-y-6">
                                ...
                                <div><Label htmlFor="desc" className="text-white mb-1 font-semibold">Descrição</Label><Textarea id="desc" className="bg-black/70 border-white/20 rounded-xl" value={editingService.description} onChange={(e) => setEditingService({ ...editingService, description: e.target.value })} rows={4} /></div>
                                <div><Label htmlFor="features" className="text-white mb-1 font-semibold">Características (separadas por vírgula)</Label><Input id="features" className="bg-black/70 border-white/20 rounded-xl h-12" value={editingService.features.join(', ')} onChange={(e) => setEditingService({ ...editingService, features: e.target.value.split(',').map(f => f.trim()) })} /></div>
                                <div><Label htmlFor="price" className="text-white mb-1 font-semibold">Preço (ex: "A partir de 500€")</Label><Input id="price" className="bg-black/70 border-white/20 rounded-xl h-12" value={editingService.price} onChange={(e) => setEditingService({ ...editingService, price: e.target.value })} /></div>
