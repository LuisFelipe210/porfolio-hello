        ...
        <section id="services" className="py-16 md:py-24 bg-background">
            <div className="container mx-auto max-w-6xl">
                ...
                <div className="relative">
                    {/* Botões de Navegação Flutuantes - Estilo mais sóbrio e destacado */}
                    ...
                    <div ref={carouselRef} className="flex md:grid md:grid-cols-2 lg:grid-cols-4 md:overflow-visible overflow-x-auto px-4 no-scrollbar snap-x snap-mandatory gap-6">
                        {isLoading ? (
                            Array.from({ length: 4 }).map((_, idx) => (
                                <Skeleton key={idx} className="flex-shrink-0 md:flex-shrink md:w-auto w-[80vw] h-[360px] rounded-xl" />
                            ))
                        ) : (
                            services.map((service, idx) => {
                                ...
                                return (
                                    // CORREÇÃO: Aumentando o padding para p-8 md:p-10
                                    <div
                                        key={service._id}
                                        className="flex-shrink-0 md:flex-shrink md:w-auto w-[80vw] p-10 md:p-8
                                            // Fundo mais limpo e claro/escuro
                                            bg-white dark:bg-zinc-900
                                            // Borda elegante e hover
                                            border border-transparent hover:border-orange-500/50
                                            // Arredondamento e sombra (menos redondo: rounded-xl)
                                            rounded-xl shadow-lg hover:shadow-2xl hover:shadow-orange-500/20 transition-all duration-300
                                            flex flex-col justify-between animate-fade-in snap-center overflow-hidden"
                                    >
                                        <div>
                                            {/* Ícone e Título - Corrigido o espaço */}
                                            <div className="flex items-center mb-5">
                                                {/* Ajuste: Contêiner menor (w-10 h-10) e margem menor (mr-3) para liberar espaço */}
                                                ...
                                                {/* Reduzindo o tamanho da fonte para text-lg (18px) e md:text-xl (20px) para caber na linha */}
                                        ...
                                        {/* Botão Outline/Ghost Orange (Menos redondo, apenas 'Valores', padding reduzido) */}
                                        <a
                                            ...
                                            // NOVO ESTILO: Outline/Ghost, arredondamento xl, padding reduzido
