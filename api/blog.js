...
// --- ID DE CONFIGURAÇÃO DE DISPONIBILIDADE ---
// Usamos uma string simples como ID (não um ObjectId) para que o upsert:true funcione na primeira vez
...
export default async function handler(req, res) {
    try {
        ...
        if (feature === 'availability') {
            ...
            // --- GET Público: Buscar Datas Reservadas ---
            if (req.method === 'GET') {
                const doc = await collection.findOne({ _id: AVAILABILITY_DOC_ID });
                // Se o documento ainda não existir, retorna um objeto com reservedDates vazio.
                return res.status(200).json({ reservedDates: doc ? doc.reservedDates : [] });
            }

            // --- POST Protegido: Atualizar Datas Reservadas ---
            if (req.method === 'POST') {
                // (1) Verifica a autenticação antes de atualizar
                const token = req.headers.authorization?.split(' ')[1];
                if (!token) return res.status(401).json({ error: 'Token não fornecido.' });

                // Erro de token JWT será capturado pelo try/catch
        ...
        if (feature === 'blog') {
            const collection = db.collection('posts'); // Coleção para artigos do blog
